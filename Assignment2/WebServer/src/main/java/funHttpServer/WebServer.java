      ...
      if (request == null) {
		...
		try {
		 ...
		 validInput = true;
		    ...
		    builder.append("Content-Type: text/html; charset=utf-8\n");
		    builder.append("\n");
		    builder.append("Need 2 integers, non-integer character detected.");
		}
          // do math
	if (validInput == true) {
	  ...
	  for (int i = 0; i < jsonArray.length(); i++) {
	    ...
	    builder.append(jsonArray.getJSONObject(i).getString("full_name"));
	    builder.append("<br>");
	    ...
	    String[] randomTitles = {"the nasty", "the brave", "the unclean", "the vigilant", "the cowardly", "the legend", "the quiet"};
	    String[] randomActions = {"killed the orange cat with a broom", "drank decaf coffee with a straw", "caught his own tail, twice", "jumped in a puddle of chocolate milk"};
	    int randTitleInt = rand.nextInt(randomTitles.length);
	    int randActionInt = rand.nextInt(randomActions.length);
	    builder.append(name1 + " " + randomTitles[randTitleInt] + " " + randomActions[randActionInt] + " and " + "<br>");
	    randTitleInt = rand.nextInt(randomTitles.length);
	    randActionInt = rand.nextInt(randomActions.length);
	    builder.append(name2 + " " + randomTitles[randTitleInt] + " " + randomActions[randActionInt]);
	    ...
	    background = query_pairs.get("bg");
	    font = query_pairs.get("font");
	    
	    builder.append("<style>");
 	    builder.append(" body {background-color: " + background + ";");
	    builder.append("color: " + font);
	    builder.append("</style>");
	    builder.append("Well, hello there.");
	    builder.append("<br>");
	    builder.append("You can also try colors with a hex value from: <a href=\"https://htmlcolorcodes.com/color-picker/\">htmlcolorcodes.com</a> without the #");
